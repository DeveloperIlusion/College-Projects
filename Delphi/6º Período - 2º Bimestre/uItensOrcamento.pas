...
//Define e chama a função que setta o tipo de ação ao abrirmos o form
procedure TfrmItensOrcamento.Carregar;
begin
  case TipoOperacao of
    topInserir:
    begin
      Caption := 'Inserindo item orçamento';
      uConexao.queryItemOrcamento.Append;
    end;
    topEditar:
    begin
      Caption := 'Editando item orçamento';
...
//Define e setta a função de cancelar o processo e fechar ao query no clique do
//botão cancelar.
procedure TfrmItensOrcamento.btnCancelarClick(Sender: TObject);
begin
  Caption := 'Itens Orçamento';
  Cancelar;
end;
...
//Define a ação de gravação e a validação dos dados
